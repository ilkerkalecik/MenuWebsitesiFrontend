...
const Sidebar = () => {
    ...
    const [opacity, setOpacity] = useState(1); // Butonun görünürlüğü için state
    ...
    // Sayfa kaydırıldıkça butonun görünürlüğünü ayarla
    useEffect(() => {
        const handleScroll = () => {
            const scrollY = window.scrollY;
            const newOpacity = Math.max(1 - scrollY / 300, 0); // 300px kayınca 0 oluyor
    ...
    return (
        <>
            {/* Sidebar dışına tıklayınca kapanmasını sağlayan arka plan */}
            {isOpen && (
                <div 
            ...
            <button 
                ...
                className="fixed top-5 right-5 z-50 bg-primaryWhite p-2 rounded-full shadow-md transition-opacity duration-300 ease-in-out"
                style={{ opacity }} // Opacity'yi dinamik olarak uygula
            >
                {isOpen ? <X size={24} /> : <Menu size={24} />}
            </button>

            {/* Sidebar */}
            <div className={`fixed top-0 right-0 h-full w-64 bg-secondaryWhite shadow-lg transform ${isOpen ? 'translate-x-0' : 'translate-x-full'} transition-transform duration-300 ease-in-out z-50`}>
                <div className="flex flex-col h-full p-5">
                    {/* Sidebar kapatma butonu */}
                    <button 
                        onClick={() => setIsOpen(false)} 
                        className="absolute top-5 left-5 bg-gray-100 p-2 rounded-full shadow-md"
                    >
                        <X size={24} />
                    </button>
