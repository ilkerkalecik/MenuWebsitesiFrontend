...
const Menu = () => {
    ...
    return (
        <div className="space-y-12">
            ...
            <div className="sticky top-0   py-4 z-10 mt-20 overflow-x-auto flex gap-2 whitespace-nowrap  scrollbar-hide  bg-secondaryWhite">
                <button
                    onClick={() => setSelectedCategory(null)}
                    className={`px-4 py-2 rounded-full  tracking-wide border border-neutral-300 transition-all ${!selectedCategory ? 'bg-mainColor text-neutral-200' : 'text-neutral-500 '}`}
                >
                    Tüm Ürünler
                </button>
                {categories.map(category => {
                    ...
                    return (
                        <button
                            ...
                            className={`flex items-center capitalize gap-2 px-4 py-2 rounded-full border border-neutral-300   tracking-wide transition-all  ${selectedCategory === category.name ? 'bg-mainColor text-neutral-200' : ' text-neutral-500'}`}
                        >
            ...
            {filteredCategories.map((category) => (
                <div key={category.id}>
                    <div className="relative flex items-center justify-center my-10">
                        ...
                        <h2 className="px-6 text-2xl md:text-3xl lg:text-4xl font-medium text-mainColor tracking-widest capitalize">
                            {category.name}
                        </h2>
                        <div className="flex-grow border-t border-neutral-400"></div>
                    </div>
                    ...
                    <div className="grid grid-cols-1 gap-6 p-4">
                        {category.products.map((product) => (
                            <div key={product.id} className="flex flex-row items-center  space-x-4 w-full">
                                <h1 className="text-neutral-700 text-xs md:text-sm lg:text-sm tracking-wider">{product.name}</h1>
                                <div className="flex-grow border-dotted border-t border-neutral-400"></div>
                                <p className="font-bold text-mainColor text-xs md:text-sm lg:text-sm">{product.price.toFixed(2)} ₺</p>
                                <button
                                    onClick={() => openModal(product)}
                                    className="text-xs md:text-sm lg:text-sm font-semibold  py-1 px-3 text-neutral-600 underline rounded-md hover:bg-primaryDark transition-all "
                                >
                                    Detaylar...
                                </button>
                            </div>
            ...
            {showScrollButton && (
                <button
                    onClick={scrollToTop}
                    className="fixed bottom-10 right-5 p-3 bg-mainColor text-white rounded-full shadow-lg transition-opacity duration-300 ease-in-out"
                >
                    <ChevronUp size={24} />
                </button>
            )}
